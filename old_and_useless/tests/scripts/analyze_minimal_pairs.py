...
project_root = Path(__file__).resolve().parents[2]  # Go up 2 levels from tests/scripts/
...
def main():
    ...
    dataset_wh = pd.read_json('tests/datasets/wh_question_dataset.json')
    dataset_orc = pd.read_json('tests/datasets/object_relative_dataset.json')
