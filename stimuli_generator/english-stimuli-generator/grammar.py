...
# Pre-compile sets for faster lookup
agentive_set = set(agentive_nouns)
verb_set = set(verb_surface)

# Use buffered writing to reduce I/O overhead
buffer = []
buffer_size = 100  # Write every 1000 sentences

with open("orc.txt", "w") as f:
    count = 0
    for p in lexicon.generate_grammar(
        "C", max_strings=10000000, n_beams=None, min_log_prob=None, max_steps=48
    ):
        sentence = irregular_map(str(p))
        ...
        # Faster duplicate checking using sets
        words = sentence.split()
        nouns_in_sentence = [w for w in words if w in agentive_set]
        verbs_in_sentence = [w for w in words if w in verb_set]
        
        # Check for duplicates
        if len(nouns_in_sentence) != len(set(nouns_in_sentence)):
            continue
        if len(verbs_in_sentence) != len(set(verbs_in_sentence)):
            continue
        ...
        if count % 100 == 0:
            print(f"Generated {count} ORC sentences")
...
print(f"Total ORC sentences: {count}")
...
#     for p in lexicon.generate_grammar(
#         "Q", max_strings=1000000, n_beams=None, min_log_prob=None, max_steps=48
#     ):
#         sentence = irregular_map(str(p))
